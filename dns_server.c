...
static int zone_upsert(const char *name,const char *mname,const char *rname,
    ...
    pthread_mutex_lock(&g_zones_mutex);
    int idx=-1;
    for(int i=0;i<g_zone_count;i++)if(strcasecmp(g_zones[i].name,name)==0){idx=i;break;}
...
static void qlog_write(const char *client_ip, const char *qname,
                       uint16_t qtype, uint8_t rcode, int answers,
                       long rtt_us, const char *transport){
    if(!g_query_log_fp&&!g_query_log_path[0]) return;
    struct timespec ts; clock_gettime(CLOCK_REALTIME,&ts);
    char line[512];
    snprintf(line,sizeof(line),
        "{\"ts\":%ld,\"client\":\"%s\",\"qname\":\"%s\",\"qtype\":\"%s\""
        ",\"rcode\":%u,\"answers\":%d,\"rtt_us\":%ld,\"transport\":\"%s\"}\n",
        (long)ts.tv_sec,client_ip,qname,type2str(qtype),
        (unsigned)rcode,answers,rtt_us,transport);
    pthread_mutex_lock(&g_qlog_mutex);
    FILE *fp=g_query_log_fp?g_query_log_fp:stderr;
    fputs(line,fp); fflush(fp);
    pthread_mutex_unlock(&g_qlog_mutex);}

static void qlog_open(void){
    pthread_mutex_lock(&g_qlog_mutex);
    if(g_query_log_fp){fclose(g_query_log_fp);g_query_log_fp=NULL;}
...
static int syslog_facility_from_str(const char *s) {
    if (!strcasecmp(s,"kern"))   return LOG_KERN;
    if (!strcasecmp(s,"user"))   return LOG_USER;
    if (!strcasecmp(s,"mail"))   return LOG_MAIL;
    if (!strcasecmp(s,"daemon")) return LOG_DAEMON;
    if (!strcasecmp(s,"auth"))   return LOG_AUTH;
    if (!strcasecmp(s,"lpr"))    return LOG_LPR;
    if (!strcasecmp(s,"news"))   return LOG_NEWS;
    if (!strcasecmp(s,"uucp"))   return LOG_UUCP;
    if (!strcasecmp(s,"cron"))   return LOG_CRON;
...
static int syslog_level_from_str(const char *s) {
    ...
    if (!strcasecmp(s,"notice"))                            return LOG_NOTICE;
    if (!strcasecmp(s,"warning")||!strcasecmp(s,"warn"))    return LOG_WARNING;
    if (!strcasecmp(s,"err")||!strcasecmp(s,"error"))       return LOG_ERR;
    if (!strcasecmp(s,"crit"))                              return LOG_CRIT;
    return LOG_INFO;
}
...
static int pri_severity(int level) {
    switch (level) {
        case LOG_EMERG:   return 0;
        case LOG_ALERT:   return 1;
        case LOG_CRIT:    return 2;
        case LOG_ERR:     return 3;
...
static int rsyslog_connect(void) {
    rsyslog_disconnect();
    if (!g_rsyslog_host[0] || g_rsyslog_port <= 0) return -1;
    ...
    if (!g_rsyslog_ssl_ctx) { close(fd); return -1; }
    ...
    if (!g_rsyslog_ssl) {
        SSL_CTX_free(g_rsyslog_ssl_ctx); g_rsyslog_ssl_ctx = NULL;
        close(fd); return -1;
    }
    ...
    if (SSL_connect(g_rsyslog_ssl) <= 0) {
        SSL_free(g_rsyslog_ssl); g_rsyslog_ssl = NULL;
        SSL_CTX_free(g_rsyslog_ssl_ctx); g_rsyslog_ssl_ctx = NULL;
        close(fd); return -1;
    }
    g_rsyslog_fd = fd;
 ...
 */
static int rsyslog_format(char *buf, int bufsz,
    ...
    if (g_rsyslog_fmt == RFMT_RFC5424) {
        ...
        return snprintf(buf, bufsz,
            "<%d>1 %s %s %s %d - - %s",
            pri, ts,
            g_rsyslog_hostname[0] ? g_rsyslog_hostname : "-",
            g_syslog_ident,
            (int)g_log_pid,
            msg);
    } else {
        ...
        return snprintf(buf, bufsz,
            "<%d>%s %s %s[%d]: %s",
            pri, ts,
            g_rsyslog_hostname[0] ? g_rsyslog_hostname : "localhost",
            g_syslog_ident,
            (int)g_log_pid,
            msg);
    }
    ...
    if (!ok) {
        ...
        if (g_rsyslog_proto == RSYSLOG_UDP) {
            ...
            if (flen + msglen < (int)sizeof(frame)) {
                memcpy(frame + flen, msg, msglen);
                flen += msglen;
                if (g_rsyslog_ssl) SSL_write(g_rsyslog_ssl, frame, flen);
                else send(g_rsyslog_fd, frame, flen, MSG_NOSIGNAL);
            }
        }
    }
}
...
static void syslog_init(void) {
    ...
    if (g_rsyslog_port > 0 && g_rsyslog_host[0]) {
        const char *proto_str =
            g_rsyslog_proto == RSYSLOG_TLS ? "tls" :
            g_rsyslog_proto == RSYSLOG_TCP ? "tcp" : "udp";
        ...
        fprintf(stdout, "[Log ] Remote syslog: %s://%s:%d  level=%s  fmt=%s\n",
                proto_str, g_rsyslog_host, g_rsyslog_port,
                syslog_level_str(g_rsyslog_level),
    ...
    dns_log(LOG_NOTICE, "[Log ] Logging initialised: local_syslog=%s remote=%s:%d\n",
            g_syslog_enabled ? "on" : "off",
            g_rsyslog_host[0] ? g_rsyslog_host : "(disabled)",
            g_rsyslog_port);
}
...
#define STAT_INC(c) do{pthread_mutex_lock(&g_stat_mutex);(c)++;pthread_mutex_unlock(&g_stat_mutex);}while(0)
...
static const char *g_local_zones[] = {
    ...
    "8.e.f.ip6.arpa","9.e.f.ip6.arpa", /* fe80::/10 LL   */
...
static int is_local_zone(const char *qname){
    ...
    for(int i=0;g_local_zones[i];i++){
        ...
        if(ql==zl&&strcmp(lq,z)==0)return 1;
        if(ql>zl+1&&lq[ql-zl-1]=='.'
           &&strcmp(lq+ql-zl,z)==0)return 1;
    }
    return 0;
}
...
static uint64_t siphash24(const uint8_t *in, int inlen, const uint8_t key[16]){
    ...
    for(i=0;i+7<inlen;i+=8){
        uint64_t m=((uint64_t)in[i])|((uint64_t)in[i+1]<<8)|((uint64_t)in[i+2]<<16)|
                   ((uint64_t)in[i+3]<<24)|((uint64_t)in[i+4]<<32)|((uint64_t)in[i+5]<<40)|
                   ((uint64_t)in[i+6]<<48)|((uint64_t)in[i+7]<<56);
        v3^=m;SIPHASH_SIPROUND(v0,v1,v2,v3);SIPHASH_SIPROUND(v0,v1,v2,v3);v0^=m;}
    uint64_t last=(uint64_t)(inlen&0xFF)<<56;
    int rem=inlen-i;
    switch(rem){
        case 7:last|=((uint64_t)in[i+6]<<48); /* fall */
        case 6:last|=((uint64_t)in[i+5]<<40); /* fall */
        case 5:last|=((uint64_t)in[i+4]<<32); /* fall */
        case 4:last|=((uint64_t)in[i+3]<<24); /* fall */
        case 3:last|=((uint64_t)in[i+2]<<16); /* fall */
        case 2:last|=((uint64_t)in[i+1]<<8);  /* fall */
        case 1:last|=((uint64_t)in[i]);        break;
        default:break;}
    v3^=last;SIPHASH_SIPROUND(v0,v1,v2,v3);SIPHASH_SIPROUND(v0,v1,v2,v3);v0^=last;
    v2^=0xFF;SIPHASH_SIPROUND(v0,v1,v2,v3);SIPHASH_SIPROUND(v0,v1,v2,v3);
 ...
 * ======================================================================= */
typedef struct {int fd;char rbuf[RESP_BUF];int rlen,rpos;} resp_conn_t;
static resp_conn_t vk;

static int resp_fill(resp_conn_t *c){
    if(c->rpos>0&&c->rlen>c->rpos){memmove(c->rbuf,c->rbuf+c->rpos,c->rlen-c->rpos);
        c->rlen-=c->rpos;c->rpos=0;}
    else if(c->rpos>=c->rlen){c->rlen=c->rpos=0;}
    int room=RESP_BUF-c->rlen-1;if(room<=0)return -1;
    int n=(int)recv(c->fd,c->rbuf+c->rlen,room,0);
    if(n<=0)return -1;c->rlen+=n;c->rbuf[c->rlen]=0;return n;}
static int resp_readline(resp_conn_t *c,char *out,int olen){
    for(;;){char *p=(char*)memchr(c->rbuf+c->rpos,'\n',c->rlen-c->rpos);
        if(p){int len=(int)(p-(c->rbuf+c->rpos));if(len>0&&*(p-1)=='\r')len--;
            if(len>=olen)len=olen-1;memcpy(out,c->rbuf+c->rpos,len);out[len]=0;
            c->rpos=(int)(p-c->rbuf)+1;return len;}
        if(resp_fill(c)<0)return -1;}}
static int resp_readbytes(resp_conn_t *c,char *out,int n){
    int got=0;
    while(got<n){int have=c->rlen-c->rpos;if(have<=0){if(resp_fill(c)<0)return -1;continue;}
        int take=(n-got<have)?(n-got):have;memcpy(out+got,c->rbuf+c->rpos,take);c->rpos+=take;got+=take;}
    while(c->rlen-c->rpos<2){if(resp_fill(c)<0)return -1;}c->rpos+=2;out[n]=0;return n;}
typedef struct{int type;long integer;char str[MAX_PEM];int count;}resp_reply_t;
...
static int resp_send(resp_conn_t *c,const char *cmd,int len){
    int sent=0;while(sent<len){int n=(int)send(c->fd,cmd+sent,len-sent,MSG_NOSIGNAL);
        if(n<=0)return -1;sent+=n;}return 0;}
static int resp_cmd(resp_conn_t *c,resp_reply_t *r,int argc,...){
    char buf[8192];int pos=snprintf(buf,sizeof(buf),"*%d\r\n",argc);
    va_list ap;va_start(ap,argc);
    for(int i=0;i<argc;i++){const char *a=va_arg(ap,const char*);int al=(int)strlen(a);
        if(pos<0||pos>=(int)sizeof(buf)){va_end(ap);return -1;}
        pos+=snprintf(buf+pos,sizeof(buf)-pos,"$%d\r\n%s\r\n",al,a);}
    va_end(ap);
    if(pos<0||pos>=(int)sizeof(buf))return -1;
    if(resp_send(c,buf,pos)<0)return -1;return resp_parse(c,r);}

static int valkey_connect_to(resp_conn_t *c,const char *host,int port,const char *pass){
    if(c->fd>=0){close(c->fd);c->fd=-1;}c->rlen=c->rpos=0;
    c->fd=socket(AF_INET,SOCK_STREAM,0);if(c->fd<0)return -1;
    struct timeval tv={.tv_sec=4};
    setsockopt(c->fd,SOL_SOCKET,SO_RCVTIMEO,&tv,sizeof(tv));
    setsockopt(c->fd,SOL_SOCKET,SO_SNDTIMEO,&tv,sizeof(tv));
    struct sockaddr_in sa={.sin_family=AF_INET,.sin_port=htons(port)};
    if(inet_pton(AF_INET,host,&sa.sin_addr)!=1){
        struct addrinfo hints={0},*res;char ps[8];snprintf(ps,sizeof(ps),"%d",port);
        hints.ai_family=AF_INET;hints.ai_socktype=SOCK_STREAM;
        if(getaddrinfo(host,ps,&hints,&res)!=0){close(c->fd);c->fd=-1;return -1;}
        memcpy(&sa,res->ai_addr,res->ai_addrlen);freeaddrinfo(res);}
    if(connect(c->fd,(struct sockaddr*)&sa,sizeof(sa))!=0){close(c->fd);c->fd=-1;return -1;}
    resp_reply_t r;
    if(pass&&pass[0]){if(resp_cmd(c,&r,2,"AUTH",pass)<0||r.type==1){close(c->fd);c->fd=-1;return -1;}}
    resp_cmd(c,&r,2,"SELECT","0");return 0;}
static int valkey_connect(resp_conn_t *c){return valkey_connect_to(c,g_valkey_host,g_valkey_port,g_valkey_pass);}
static int valkey_ensure(resp_conn_t *c){
    if(c->fd<0)return valkey_connect(c);
    resp_reply_t r;if(resp_cmd(c,&r,1,"PING")<0)return valkey_connect(c);return 0;}

static int vk_set(const char *key,const char *val,uint32_t ttl){
    pthread_mutex_lock(&g_vk_mutex);
    if(valkey_ensure(&vk)<0){pthread_mutex_unlock(&g_vk_mutex);return 0;}
    resp_reply_t r;int ok;
    if(ttl>0){char ts[32];snprintf(ts,sizeof(ts),"%u",ttl);ok=resp_cmd(&vk,&r,5,"SET",key,val,"EX",ts);}
    else ok=resp_cmd(&vk,&r,3,"SET",key,val);
    if(ok<0){vk.fd=-1;pthread_mutex_unlock(&g_vk_mutex);return 0;}
    pthread_mutex_unlock(&g_vk_mutex);return r.type==0;}
static int vk_get(const char *key,char *out,int olen){
    ...
    /* Write stale shadow key: 7-day TTL — used when Valkey is unreachable */
    {
        ...
        pthread_mutex_lock(&g_vk_mutex);
        if(valkey_ensure(&vk)>=0){
            char ts[16];snprintf(ts,sizeof(ts),"%u",7*86400);
            resp_reply_t sw;
...
static int vk_del(const char *key){
    pthread_mutex_lock(&g_vk_mutex);
    if(valkey_ensure(&vk)<0){pthread_mutex_unlock(&g_vk_mutex);return 0;}
    resp_reply_t r;if(resp_cmd(&vk,&r,2,"DEL",key)<0){vk.fd=-1;pthread_mutex_unlock(&g_vk_mutex);return 0;}
    pthread_mutex_unlock(&g_vk_mutex);return r.type==3?(int)r.integer:0;}
static long vk_ttl(const char *key){
    pthread_mutex_lock(&g_vk_mutex);
    if(valkey_ensure(&vk)<0){pthread_mutex_unlock(&g_vk_mutex);return -1;}
    resp_reply_t r;if(resp_cmd(&vk,&r,2,"TTL",key)<0){vk.fd=-1;pthread_mutex_unlock(&g_vk_mutex);return -1;}
    pthread_mutex_unlock(&g_vk_mutex);return r.type==3?r.integer:-1;}

/* Load all zone_table:* keys from Valkey into the zone table.
 * Uses inline KEYS scan (no vk_keys_scan wrapper needed). */
static void zones_load_from_valkey(void){
    pthread_mutex_lock(&g_vk_mutex);
    if(valkey_ensure(&vk)<0){pthread_mutex_unlock(&g_vk_mutex);return;}
    resp_reply_t r;
    resp_cmd(&vk,&r,2,"KEYS","zone_table:*");
    if(r.type!=5){pthread_mutex_unlock(&g_vk_mutex);return;}
    int count=r.count;
    ...
    for(int i=0;i<nk;i++){
        ...
        zone_upsert(zname,
            f[0]?f[0]:"",f[1]?f[1]:"",
            f[2]?(uint32_t)atoi(f[2]):1,
            f[3]?(uint32_t)atoi(f[3]):3600,
            f[4]?(uint32_t)atoi(f[4]):900,
            f[5]?(uint32_t)atoi(f[5]):604800,
            f[6]?(uint32_t)atoi(f[6]):300,
            f[7]?f[7]:NULL,f[8]?f[8]:NULL);}}
/* Atomically increment and return new integer value */
static long vk_incr(const char *key){
    pthread_mutex_lock(&g_vk_mutex);
    if(valkey_ensure(&vk)<0){pthread_mutex_unlock(&g_vk_mutex);return -1;}
    resp_reply_t r;if(resp_cmd(&vk,&r,2,"INCR",key)<0){vk.fd=-1;pthread_mutex_unlock(&g_vk_mutex);return -1;}
    pthread_mutex_unlock(&g_vk_mutex);return r.type==3?r.integer:-1;}
...
static void config_load_from_valkey(void){
    ...
    G("nsid",g_nsid);
    G("axfr_allow",g_axfr_allow);G("notify_targets",g_notify_targets);
    ...
    /* RRL */
    {char rrl_val[16];
     if(vk_get("config:rrl_enabled",rrl_val,sizeof(rrl_val)))g_rrl_enabled=atoi(rrl_val);
     if(vk_get("config:rrl_rate",   rrl_val,sizeof(rrl_val)))g_rrl_rate   =atoi(rrl_val);
     if(vk_get("config:rrl_window", rrl_val,sizeof(rrl_val)))g_rrl_window =atoi(rrl_val);
     if(vk_get("config:rrl_slip",   rrl_val,sizeof(rrl_val)))g_rrl_slip   =atoi(rrl_val);}
    /* TSIG secret (base64) */
    ...
    if(!g_nsid[0])gethostname(g_nsid,sizeof(g_nsid)-1);
    /* PEM blobs */
    ...
    if(!g_rsyslog_hostname[0]) gethostname(g_rsyslog_hostname,sizeof(g_rsyslog_hostname)-1);
    /* Record PID */
    g_log_pid = getpid();
    syslog_init();
    #undef G
...
static uint32_t serial_bump(void){
    ...
    if(ns>0){pthread_mutex_lock(&g_soa_mutex);g_soa_serial=(uint32_t)ns;
        pthread_mutex_unlock(&g_soa_mutex);return g_soa_serial;}
    pthread_mutex_lock(&g_soa_mutex);g_soa_serial++;pthread_mutex_unlock(&g_soa_mutex);
    char buf[32];snprintf(buf,sizeof(buf),"%u",g_soa_serial);
    vk_set("config:zone_serial",buf,0);return g_soa_serial;}

/* ==========================================================================
 * TLS contexts from in-memory PEM
 * ======================================================================= */
static SSL_CTX *tls_ctx_from_pem(const char *cert_pem,const char *key_pem,
                                  const char *ca_pem,int verify_client){
    ...
    if(!cert){BIO_free(bc);SSL_CTX_free(ctx);dns_log(LOG_ERR,"[TLS] Bad cert PEM\n");return NULL;}
    SSL_CTX_use_certificate(ctx,cert);X509_free(cert);
    X509 *ca;while((ca=PEM_read_bio_X509(bc,NULL,NULL,NULL))!=NULL)SSL_CTX_add_extra_chain_cert(ctx,ca);
    BIO_free(bc);
    BIO *bk=BIO_new_mem_buf(key_pem,-1);EVP_PKEY *pkey=PEM_read_bio_PrivateKey(bk,NULL,NULL,NULL);
    BIO_free(bk);if(!pkey){SSL_CTX_free(ctx);return NULL;}
    SSL_CTX_use_PrivateKey(ctx,pkey);EVP_PKEY_free(pkey);
    if(!SSL_CTX_check_private_key(ctx)){SSL_CTX_free(ctx);return NULL;}
    if(verify_client&&ca_pem&&ca_pem[0]){
        X509_STORE *store=SSL_CTX_get_cert_store(ctx);
        BIO *bca=BIO_new_mem_buf(ca_pem,-1);X509 *cax;
        while((cax=PEM_read_bio_X509(bca,NULL,NULL,NULL))!=NULL){X509_STORE_add_cert(store,cax);X509_free(cax);}
        BIO_free(bca);SSL_CTX_set_verify(ctx,SSL_VERIFY_PEER|SSL_VERIFY_FAIL_IF_NO_PEER_CERT,NULL);}
    return ctx;}
...
static int ec_pub_xy(EVP_PKEY *k,uint8_t xy[64]){
    ...
    if(EVP_PKEY_get_bn_param(k,"qx",&x)!=1)return 0;
    if(EVP_PKEY_get_bn_param(k,"qy",&y)!=1){BN_free(x);return 0;}
    BN_bn2binpad(x,xy,32);BN_bn2binpad(y,xy+32,32);BN_free(x);BN_free(y);return 1;}
static int dnskey_rdata_ecdsa(EVP_PKEY *k,uint8_t *rd,int rdlen){
    if(rdlen<68)return -1;
    rd[0]=DNS_DNSKEY_FLAG_ZSK>>8;rd[1]=DNS_DNSKEY_FLAG_ZSK&0xFF;rd[2]=3;rd[3]=DNS_ALG_ECDSAP256SHA256;
    if(!ec_pub_xy(k,rd+4))return -1;return 68;}
static int dnskey_rdata_ed25519(EVP_PKEY *k,uint8_t *rd,int rdlen){
    if(rdlen<36)return -1;
    rd[0]=DNS_DNSKEY_FLAG_ZSK>>8;rd[1]=DNS_DNSKEY_FLAG_ZSK&0xFF;rd[2]=3;rd[3]=DNS_ALG_ED25519;
    size_t pub_len=32;
    if(EVP_PKEY_get_raw_public_key(k,rd+4,&pub_len)!=1)return -1;
    return 36;}
/* KSK variants — flag 257 */
static int dnskey_rdata_ksk_ecdsa(EVP_PKEY *k,uint8_t *rd,int rdlen){
    if(rdlen<68)return -1;
    rd[0]=DNS_DNSKEY_FLAG_KSK>>8;rd[1]=DNS_DNSKEY_FLAG_KSK&0xFF;
    rd[2]=3;rd[3]=DNS_ALG_ECDSAP256SHA256;
    if(!ec_pub_xy(k,rd+4))return -1;return 68;}
static int dnskey_rdata_ksk_ed25519(EVP_PKEY *k,uint8_t *rd,int rdlen){
    if(rdlen<36)return -1;
    rd[0]=DNS_DNSKEY_FLAG_KSK>>8;rd[1]=DNS_DNSKEY_FLAG_KSK&0xFF;
    rd[2]=3;rd[3]=DNS_ALG_ED25519;
    size_t pub_len=32;
    if(EVP_PKEY_get_raw_public_key(k,rd+4,&pub_len)!=1)return -1;
    return 36;}

static uint16_t keytag(const uint8_t *rd,int rdlen){
    uint32_t ac=0;for(int i=0;i<rdlen;i++)ac+=(i&1)?rd[i]:(uint32_t)rd[i]<<8;
    ac+=(ac>>16)&0xFFFF;return(uint16_t)(ac&0xFFFF);}
static int label_count(const char *n){
    if(!n||!*n)return 0;
    const char *start=n;
    int c=1;for(;*n;n++)if(*n=='.')c++;
    if(n>start&&n[-1]=='.')c--;
    return c;}
static int ecdsa_der_to_raw(const uint8_t *der,int dl,uint8_t raw[64]){
    const uint8_t *p=der;ECDSA_SIG *s=d2i_ECDSA_SIG(NULL,&p,(long)dl);if(!s)return 0;
    const BIGNUM *r,*sg;ECDSA_SIG_get0(s,&r,&sg);
    BN_bn2binpad(r,raw,32);BN_bn2binpad(sg,raw+32,32);ECDSA_SIG_free(s);return 1;}

/* Build RRSIG rdata for one RR, using given ZSK key + algorithm */
static int make_rrsig(const char *owner,uint16_t rrtype,uint32_t ttl,
                      const uint8_t *rrdata,uint16_t rrdata_len,
                      EVP_PKEY *zsk,uint8_t alg,uint16_t tag,uint8_t *outbuf){
    if(!zsk)return -1;
    uint8_t hdr[512];int hp=0;
    time_t now=time(NULL);
    uint32_t t_inc=(uint32_t)(now-300),t_exp=(uint32_t)(now+DNSSEC_SIG_VALIDITY);
    hdr[hp++]=rrtype>>8;hdr[hp++]=rrtype&0xFF;hdr[hp++]=alg;
    hdr[hp++]=(uint8_t)label_count(owner);
    hdr[hp++]=ttl>>24;hdr[hp++]=(ttl>>16)&0xFF;hdr[hp++]=(ttl>>8)&0xFF;hdr[hp++]=ttl&0xFF;
    hdr[hp++]=t_exp>>24;hdr[hp++]=(t_exp>>16)&0xFF;hdr[hp++]=(t_exp>>8)&0xFF;hdr[hp++]=t_exp&0xFF;
    ...
    rr[rp++]=rrdata_len>>8;rr[rp++]=rrdata_len&0xFF;
    ...
    if(!der){EVP_MD_CTX_free(mc);return -1;}
    EVP_DigestSignFinal(mc,der,&sl);EVP_MD_CTX_free(mc);
    memcpy(outbuf,hdr,hp);
    if(alg==DNS_ALG_ED25519){
        memcpy(outbuf+hp,der,sl);free(der);return hp+(int)sl;}
    uint8_t raw[64];
    if(!ecdsa_der_to_raw(der,sl,raw)){free(der);return -1;}
    free(der);memcpy(outbuf+hp,raw,64);return hp+64;}
...
static void dnssec_init_key(const char *vk_key,EVP_PKEY **out,uint16_t *tag,int alg,
    ...
    if(vk_get(vk_key,pem,sizeof(pem))&&strlen(pem)>10){
        ...
        *out=PEM_read_bio_PrivateKey(b,NULL,NULL,NULL);
        pthread_mutex_unlock(&g_zsk_mutex);BIO_free(b);}
    if(!*out){
        ...
        else{kctx=EVP_PKEY_CTX_new_id(EVP_PKEY_EC,NULL);
            EVP_PKEY_keygen_init(kctx);
            EVP_PKEY_CTX_set_ec_paramgen_curve_nid(kctx,NID_X9_62_prime256v1);}
        pthread_mutex_lock(&g_zsk_mutex);EVP_PKEY_keygen(kctx,out);pthread_mutex_unlock(&g_zsk_mutex);
        EVP_PKEY_CTX_free(kctx);
        BIO *b=BIO_new(BIO_s_mem());
        PEM_write_bio_PrivateKey(b,*out,NULL,NULL,0,NULL,NULL);
        char *pp;long pl=BIO_get_mem_data(b,&pp);
        char pem2[MAX_PEM];int n=(int)(pl<MAX_PEM-1?pl:MAX_PEM-1);
        memcpy(pem2,pp,n);pem2[n]=0;vk_set(vk_key,pem2,0);BIO_free(b);
    ...
    if(dklen>0)*tag=keytag(dkrd,dklen);
    dns_log(LOG_INFO,"[DNSSEC] %s key tag: %u\n",label,*tag);}
static void dnssec_init(void){
    pthread_mutex_lock(&g_zsk_mutex);
    g_zsk=NULL;g_zsk_ed=NULL;g_ksk=NULL;g_ksk_ed=NULL;
...
static void nsec3_hash_name(const char *name,const uint8_t *salt,int saltlen,
    ...
    uint8_t h[20];
    uint8_t ibuf[256+64];int iblen=wlen;
    memcpy(ibuf,wire,wlen);memcpy(ibuf+wlen,salt,saltlen);iblen+=saltlen;
    sha1(ibuf,iblen,h);
    for(int i=0;i<iters;i++){memcpy(ibuf,h,20);memcpy(ibuf+20,salt,saltlen);
        sha1(ibuf,20+saltlen,h);}
    base32hex_enc(h,20,out_b32hex,out_len);}

/* Build NSEC3 RDATA for a name, covering given type bitmap */
static int nsec3_rdata(const char *name, uint16_t covered_type,
                       uint8_t *out, int outlen){
    uint8_t salt[16];int saltlen=0;
    if(g_nsec3_salt[0])saltlen=hex_dec(g_nsec3_salt,salt,sizeof(salt));
    /* Compute next-closer hash (we use the same name hash as next for simplicity) */
    char hash[64];nsec3_hash_name(name,salt,saltlen,g_nsec3_iters,hash,sizeof(hash));
    ...
    {uint8_t ib[256+64];int ib_len=zw;memcpy(ib,zone_wire,zw);
     ...
     for(int i=0;i<g_nsec3_iters;i++){uint8_t ib2[256+64];memcpy(ib2,nexthash,20);
         if(saltlen){memcpy(ib2+20,salt,saltlen);}sha1(ib2,20+saltlen,nexthash);}}
    if(outlen<25)return -1;
    int op=0;
    out[op++]=NSEC3_ALG_SHA1;out[op++]=0;/* flags: opt-out=0 */
    out[op++]=(uint16_t)g_nsec3_iters>>8;out[op++]=(uint16_t)g_nsec3_iters&0xFF;
    out[op++]=(uint8_t)saltlen;memcpy(out+op,salt,saltlen);op+=saltlen;
    out[op++]=20;memcpy(out+op,nexthash,20);op+=20;
    /* Type bitmap: Window 0, block covering covered_type */
    if(covered_type<256&&op+4<outlen){
        uint8_t bmap[32]={0};bmap[covered_type/8]|=(0x80>>(covered_type%8));
        int blen=covered_type/8+1;
        out[op++]=0;out[op++]=(uint8_t)blen;memcpy(out+op,bmap,blen);op+=blen;}
    return op;}
...
static const uint8_t *tsig_find(const uint8_t *pkt,int plen,tsig_rr_t *t){
    if(plen<12)return NULL;
    const dns_hdr_t *h=(const dns_hdr_t*)pkt;
    int ar=ntohs(h->arcount);if(ar==0)return NULL;
    /* Skip header, questions, answers, authority */
    int off=12;
    int skip=ntohs(h->qdcount)+ntohs(h->ancount)+ntohs(h->nscount);
    for(int i=0;i<skip;i++){
        char name[256];int a=/* name_from_wire inline: */ off;
        /* skip name */
        for(;;){if(a>=plen)return NULL;uint8_t c=pkt[a];
            if((c&0xC0)==0xC0){a+=2;break;}if(c==0){a++;break;}a+=c+1;}
        if(a+4>plen)return NULL;a+=4;/* type+class */
        /* skip rdlen if RR (not question) */
        if(i>=ntohs(h->qdcount)){if(a+6>plen)return NULL;
            uint16_t rdlen=((uint16_t)pkt[a+4]<<8)|pkt[a+5];a+=6+rdlen;}
        off=a;}
    /* Scan additional RRs for TSIG */
    for(int i=0;i<ar;i++){
        char name[256];int a=off;int orig_off=a;
        /* skip name */
        for(;;){if(a>=plen)return NULL;uint8_t c=pkt[a];
            if((c&0xC0)==0xC0){a+=2;break;}if(c==0){a++;break;}a+=c+1;}
        if(a+10>plen)return NULL;
        uint16_t rtype=((uint16_t)pkt[a]<<8)|pkt[a+1];
        uint16_t rdlen=((uint16_t)pkt[a+8]<<8)|pkt[a+9];
        if(rtype==250/*TSIG*/){
            /* Parse TSIG RDATA */
            int rd_off=a+10;if(rd_off+rdlen>plen)return NULL;
            /* key name already in name[] – re-parse */
            {const uint8_t *p2=pkt+orig_off;int opos=0;
             for(;;){uint8_t c2=*p2;if(c2==0){break;}
                 if((c2&0xC0)==0xC0){p2+=2;break;}
                 /* bounds: dot + label (c2 bytes) must fit in 255 chars */
                 int dot=(opos>0)?1:0;
                 if(opos+dot+c2>=(int)sizeof(t->key_name)-1){opos=0;break;}
                 if(dot)t->key_name[opos++]='.';
                 memcpy(t->key_name+opos,p2+1,c2);opos+=c2;p2+=c2+1;}
             t->key_name[opos]=0;strlower(t->key_name);}
            /* capture alg name */
            int rp=rd_off;
            {int apos=0;
             for(;;){if(rp>=plen)return NULL;uint8_t c=pkt[rp];
                 if(c==0){rp++;break;}
                 int dot=(apos>0)?1:0;
                 if(apos+dot+(int)c<(int)sizeof(t->alg_name)-1){
                     if(dot)t->alg_name[apos++]='.';
                     memcpy(t->alg_name+apos,pkt+rp+1,c);apos+=c;}
                 rp+=c+1;}
             if(apos<(int)sizeof(t->alg_name))t->alg_name[apos]='.';
             t->alg_name[sizeof(t->alg_name)-1]=0;strlower(t->alg_name);}
            if(rp+10>plen)return NULL;
            t->time_high=((uint32_t)pkt[rp]<<8)|pkt[rp+1];rp+=2;
...
static const char *tsig_alg_to_digest(const char *alg_name){
    if(!alg_name)return "SHA256";
    if(strcasestr(alg_name,"sha512"))return "SHA512";
    if(strcasestr(alg_name,"sha384"))return "SHA384";
    if(strcasestr(alg_name,"sha256"))return "SHA256";
    if(strcasestr(alg_name,"sha1"))  return "SHA1";
    if(strcasestr(alg_name,"md5"))   return "MD5";
...
static int tsig_verify(const uint8_t *pkt,int plen){
    ...
    memcpy(vars+vp,alg_wire,13);vp+=13;
    /* time signed */
    vars[vp++]=(uint8_t)(t.time_high>>8);vars[vp++]=(uint8_t)(t.time_high&0xFF);
    vars[vp++]=(uint8_t)((t.time_low>>24)&0xFF);vars[vp++]=(uint8_t)((t.time_low>>16)&0xFF);
    vars[vp++]=(uint8_t)((t.time_low>>8)&0xFF);vars[vp++]=(uint8_t)(t.time_low&0xFF);
...
static int tsig_append(uint8_t *buf,int off,int blen,uint16_t orig_id,uint16_t error){
    if(g_tsig_secret_len==0)return off;
    ...
    uint8_t rdata[200];int rp=0;
    memcpy(rdata+rp,alg_wire,13);rp+=13;
    ...
    rdata[rp++]=error>>8;rdata[rp++]=error&0xFF;
    rdata[rp++]=0;rdata[rp++]=0;/* other len=0 */
    buf[off++]=0;buf[off++]=(uint8_t)rp;
    memcpy(buf+off,rdata,rp);off+=rp;
    /* Increment arcount */
...
static const char *type2str(uint16_t t){
    ...
    case DNS_TYPE_CNAME:return"CNAME";case DNS_TYPE_SOA:return"SOA";
    case DNS_TYPE_MX:return"MX";case DNS_TYPE_TXT:return"TXT";
    case DNS_TYPE_AAAA:return"AAAA";case DNS_TYPE_LOC:return"LOC";
    case DNS_TYPE_SRV:return"SRV";case DNS_TYPE_DNAME:return"DNAME";
...
static int build_soa_rdata(uint8_t *rd,int rdlen){
    int pos=0;
    int n=name_to_wire(g_soa_mname,rd,rdlen);if(n<0)return -1;pos+=n;
    n=name_to_wire(g_soa_rname,rd+pos,rdlen-pos);if(n<0)return -1;pos+=n;
    if(pos+20>rdlen)return -1;
    pthread_mutex_lock(&g_soa_mutex);
    put32(rd,pos,g_soa_serial);pos+=4;
    put32(rd,pos,g_soa_refresh);pos+=4;
    put32(rd,pos,g_soa_retry);pos+=4;
    put32(rd,pos,g_soa_expire);pos+=4;
    put32(rd,pos,g_soa_minimum);pos+=4;
...
static void edns_parse(const uint8_t *pkt,int plen,edns_info_t *ei){
    memset(ei,0,sizeof(*ei));ei->max_udp=512;
    const dns_hdr_t *h=(const dns_hdr_t*)pkt;
    if(ntohs(h->arcount)==0)return;
    ...
    for(int s=0;s<3;s++){
        ...
        for(int i=0;i<cnt;i++){
            for(;;){if(off>=plen)return;uint8_t c=pkt[off];
                if((c&0xC0)==0xC0){off+=2;break;}if(c==0){off++;break;}off+=c+1;}
            if(s==0){off+=4;}else{if(off+10>plen)return;uint16_t rdl=get16(pkt,off+8);off+=10+rdl;}}}
    for(int i=0;i<ntohs(h->arcount);i++){
        if(off>=plen)break;
        int name_start=off;(void)name_start;
        for(;;){if(off>=plen)return;uint8_t c=pkt[off];
            if((c&0xC0)==0xC0){off+=2;break;}if(c==0){off++;break;}off+=c+1;}
        if(off+10>plen)break;
        uint16_t rtype=get16(pkt,off);
        if(rtype==DNS_TYPE_OPT){
            ei->present=1;ei->max_udp=get16(pkt,off+2);
            ei->version=pkt[off+5];
            ei->do_bit=(pkt[off+4]&0x80)?1:0;
            uint16_t rdlen=get16(pkt,off+8);int rp=off+10;
            while(rp+4<=off+10+rdlen&&rp+4<=plen){
                ...
                rp+=ol;}
            break;}
        uint16_t rdlen=get16(pkt,off+8);off+=10+rdlen;}
}
...
static int edns_append_opt(uint8_t *buf,int off,int blen,int is_tcp,
    ...
    buf[off++]=0;
    /* type = OPT */
    buf[off++]=0;buf[off++]=41;
    /* class = max UDP payload */
    uint16_t maxudp=is_tcp?65535:EDNS_MAX_UDP;
    buf[off++]=maxudp>>8;buf[off++]=maxudp&0xFF;
    /* Extended RCODE (top 8 bits) | version=0 | DO bit | Z */
    buf[off++]=rcode_ext&0xFF;buf[off++]=0;
    buf[off++]=do_bit?0x80:0x00;buf[off++]=0;
    /* RDATA: options */
    int rdata_off=off+2;int rdata_len=0;
    if(off+2>blen)return off;
    /* NSID option */
    if(req_ei&&req_ei->nsid_req&&g_nsid[0]){
        int nsid_len=(int)strlen(g_nsid);
        if(rdata_off+4+nsid_len<blen){
            put16(buf,rdata_off,EDNS_OPT_NSID);put16(buf,rdata_off+2,(uint16_t)nsid_len);
            memcpy(buf+rdata_off+4,g_nsid,nsid_len);
            rdata_off+=4+nsid_len;rdata_len+=4+nsid_len;}}
    /* Cookie option */
    ...
    if(is_tcp&&req_ei&&req_ei->keepalive_req){
        if(rdata_off+6<blen){
            put16(buf,rdata_off,EDNS_OPT_KEEPALIVE);put16(buf,rdata_off+2,2);
            put16(buf,rdata_off+4,300);/* 30s in units of 100ms */
            rdata_off+=6;rdata_len+=6;}}
    /* EDE option */
    if(ede_code>=0){
        int tlen=ede_text?(int)strlen(ede_text):0;
        if(rdata_off+4+2+tlen<blen){
            put16(buf,rdata_off,EDNS_OPT_EDE);put16(buf,rdata_off+2,(uint16_t)(2+tlen));
            put16(buf,rdata_off+4,(uint16_t)ede_code);
            if(tlen)memcpy(buf+rdata_off+6,ede_text,tlen);
            rdata_off+=4+2+tlen;rdata_len+=4+2+tlen;}}
    ...
    if(is_tcp){
        ...
        if(pad==block)pad=0; /* already on a block boundary */
        if(pad>0&&rdata_off+4+pad<blen){
            ...
            rdata_off+=4+pad;rdata_len+=4+pad;
        }
    }
    /* Write RDLEN */
    put16(buf,off,(uint16_t)rdata_len);off=rdata_off;
    /* Increment arcount */
    dns_hdr_t *rh=(dns_hdr_t*)buf;rh->arcount=htons(ntohs(rh->arcount)+1);
    return off;}
...
static int emit_rr(uint8_t *resp,int off,int resp_len,const char *name,
                   uint16_t type,uint32_t ttl,const uint8_t *rdata,uint16_t rdlen,
...
static int nsec_rdata(const char *qname, uint8_t *rd, int rdlen){
    ...
    if(pos+2+blen>rdlen)return -1;
    rd[pos++]=0;      /* window block 0 */
    rd[pos++]=(uint8_t)blen;
    memcpy(rd+pos,bitmap,blen);pos+=blen;
    return pos;}

static int add_nsec_denial(uint8_t *resp,int off,int resp_len,
                            const char *qname,int dnssec_ok,int *auth_count){
    uint8_t rd[512];int rdlen=nsec_rdata(qname,rd,sizeof(rd));
    if(rdlen<0)return off;
    off=emit_rr(resp,off,resp_len,qname,DNS_TYPE_NSEC,g_soa_minimum,
                rd,(uint16_t)rdlen,dnssec_ok,auth_count);
    return off;}

/* ==========================================================================
 * Query response builder
 * ======================================================================= */
static int emit_rr(uint8_t *resp,int off,int resp_len,
                   const char *name,uint16_t type,uint32_t ttl,
                   const uint8_t *rdata,uint16_t rdlen,int dnssec_ok,int *answers){
    ttl=ttl_clamp(ttl); /* RFC 2181 §8 */
    int noff=append_rr(resp,off,resp_len,name,type,DNS_CLASS_IN,ttl,rdata,rdlen);
    if(noff<0)return off;(*answers)++;off=noff;
    if(!dnssec_ok||!g_zsk)return off;
    /* Add RRSIG for ECDSA P-256 (Alg 13) */
    STAT_INC(g_stat_signed);
    {pthread_mutex_lock(&g_zsk_mutex);EVP_PKEY *zsk=g_zsk;uint16_t tag=g_zsk_tag;pthread_mutex_unlock(&g_zsk_mutex);
     if(zsk){uint8_t sig[512];int sl=make_rrsig(name,type,ttl,rdata,rdlen,zsk,DNS_ALG_ECDSAP256SHA256,tag,sig);
         if(sl>0){int so=append_rr(resp,off,resp_len,name,DNS_TYPE_RRSIG,DNS_CLASS_IN,ttl,sig,(uint16_t)sl);
             if(so>0){off=so;(*answers)++;}}}}
    /* Add RRSIG for Ed25519 (Alg 15) */
    {pthread_mutex_lock(&g_zsk_mutex);EVP_PKEY *zsk=g_zsk_ed;uint16_t tag=g_zsk_ed_tag;pthread_mutex_unlock(&g_zsk_mutex);
     if(zsk){uint8_t sig[512];int sl=make_rrsig(name,type,ttl,rdata,rdlen,zsk,DNS_ALG_ED25519,tag,sig);
         if(sl>0){int so=append_rr(resp,off,resp_len,name,DNS_TYPE_RRSIG,DNS_CLASS_IN,ttl,sig,(uint16_t)sl);
             if(so>0){off=so;(*answers)++;}}}}
    return off;}
...
static int add_soa_authority(uint8_t *resp,int off,int resp_len,int dnssec_ok,int *auth_count){
    uint8_t soa_rd[512];int soa_len=build_soa_rdata(soa_rd,sizeof(soa_rd));
    if(soa_len<0)return off;
    off=emit_rr(resp,off,resp_len,g_zone_name,DNS_TYPE_SOA,g_soa_minimum,
                soa_rd,(uint16_t)soa_len,dnssec_ok,auth_count);
    return off;}

/* Add NSEC3 to authority for authenticated denial */
static int add_nsec3_denial(uint8_t *resp,int off,int resp_len,
                             const char *qname,uint16_t qtype,int dnssec_ok,int *auth_count){
    uint8_t n3rd[256];int n3len=nsec3_rdata(qname,qtype,n3rd,sizeof(n3rd));
    if(n3len>0){
        /* NSEC3 owner name = base32hex_hash.zone */
        uint8_t salt[16];int saltlen=0;
        if(g_nsec3_salt[0])saltlen=hex_dec(g_nsec3_salt,salt,sizeof(salt));
        char hash[64];nsec3_hash_name(qname,salt,saltlen,g_nsec3_iters,hash,sizeof(hash));
        char nsec3_owner[512];snprintf(nsec3_owner,sizeof(nsec3_owner),"%s.%s",hash,g_zone_name);
        strlower(nsec3_owner);
        off=emit_rr(resp,off,resp_len,nsec3_owner,DNS_TYPE_NSEC3,g_soa_minimum,
                    n3rd,(uint16_t)n3len,dnssec_ok,auth_count);}
    return off;}
...
static int build_query_resp(const uint8_t *query,int qlen,uint8_t *resp,int resp_len,
    ...
    if(ntohs(qh->qdcount)!=1){
        rh->id=qh->id;rh->flags=htons(DNS_QR|DNS_RCODE_FORMERR);
        rh->qdcount=rh->ancount=rh->nscount=rh->arcount=0;
        return 12;}
    STAT_INC(g_stat_queries);
    rh->id=qh->id;rh->flags=htons(DNS_QR|DNS_AA|DNS_RCODE_NOERROR);
    rh->qdcount=htons(1);rh->ancount=rh->nscount=rh->arcount=0;
    int off=12;
    char qname[256]={0};
    int after=name_from_wire(query,qlen,off,qname,sizeof(qname));
    if(after<0||after+3>=qlen)return -1;
    int qsec=after-off+4;if(off+qsec>resp_len)return -1;
     ...
     * names to us.  Locally-served zones (RFC 6303) are exempt. */
    {
        ...
        if(g_zone_name[0]){
            ...
            if(streq_ci(qname,g_zone_name)) in_zone=1;
            else if(ql>zl+1&&qname[ql-zl-1]=='.'
                    &&streq_ci(qname+ql-zl,g_zone_name)) in_zone=1;
        }
    ...
    if(qtype==DNS_TYPE_ANY)any_minimal=1;
    ...
    if((qtype==DNS_TYPE_DNSKEY||qtype==DNS_TYPE_ANY)){
        ...
        if(g_zsk&&dnskey_rdata_ecdsa(g_zsk,dkrd,sizeof(dkrd))>0){found=1;
            int n2=append_rr(resp,off,resp_len,qname,DNS_TYPE_DNSKEY,
                DNS_CLASS_IN,3600,dkrd,68);
            if(n2>0){off=n2;answers++;}}
        /* ZSK Ed25519 */
        if(g_zsk_ed&&dnskey_rdata_ed25519(g_zsk_ed,dkrd,sizeof(dkrd))>0){found=1;
            int n2=append_rr(resp,off,resp_len,qname,DNS_TYPE_DNSKEY,
                DNS_CLASS_IN,3600,dkrd,36);
            if(n2>0){off=n2;answers++;}}
        /* KSK P-256 — emit then sign DNSKEY RRset with KSK */
        if(g_ksk&&dnskey_rdata_ksk_ecdsa(g_ksk,dkrd,sizeof(dkrd))>0){found=1;
            int n2=append_rr(resp,off,resp_len,qname,DNS_TYPE_DNSKEY,
                DNS_CLASS_IN,3600,dkrd,68);
            if(n2>0){off=n2;answers++;}
            /* Sign DNSKEY with KSK */
            if(dnssec_ok){
                uint8_t sig[512];int sl=make_rrsig(qname,DNS_TYPE_DNSKEY,3600,
                    dkrd,68,g_ksk,DNS_ALG_ECDSAP256SHA256,g_ksk_tag,sig);
                if(sl>0){int so=append_rr(resp,off,resp_len,qname,DNS_TYPE_RRSIG,
                    DNS_CLASS_IN,3600,sig,(uint16_t)sl);
                    if(so>0){off=so;answers++;}}}}
        /* KSK Ed25519 */
        if(g_ksk_ed&&dnskey_rdata_ksk_ed25519(g_ksk_ed,dkrd,sizeof(dkrd))>0){found=1;
            int n2=append_rr(resp,off,resp_len,qname,DNS_TYPE_DNSKEY,
                DNS_CLASS_IN,3600,dkrd,36);
            if(n2>0){off=n2;answers++;}
            if(dnssec_ok){
                uint8_t sig[512];int sl=make_rrsig(qname,DNS_TYPE_DNSKEY,3600,
                    dkrd,36,g_ksk_ed,DNS_ALG_ED25519,g_ksk_ed_tag,sig);
                if(sl>0){int so=append_rr(resp,off,resp_len,qname,DNS_TYPE_RRSIG,
                    DNS_CLASS_IN,3600,sig,(uint16_t)sl);
                    if(so>0){off=so;answers++;}}}}
    ...
    if(qtype==DNS_TYPE_CDS||qtype==DNS_TYPE_ANY){
        uint8_t dkrd[68];
        if(g_zsk&&dnskey_rdata_ecdsa(g_zsk,dkrd,sizeof(dkrd))>0){found=1;
            off=emit_rr(resp,off,resp_len,qname,DNS_TYPE_CDS,3600,dkrd,68,dnssec_ok,&answers);}
        if(g_zsk_ed&&dnskey_rdata_ed25519(g_zsk_ed,dkrd,sizeof(dkrd))>0){found=1;
    ...
    if(qtype==DNS_TYPE_CDNSKEY||qtype==DNS_TYPE_ANY){
        uint8_t dkrd[68];
        if(g_zsk&&dnskey_rdata_ecdsa(g_zsk,dkrd,sizeof(dkrd))>0){found=1;
            off=emit_rr(resp,off,resp_len,qname,DNS_TYPE_CDNSKEY,3600,dkrd,68,dnssec_ok,&answers);}
        if(g_zsk_ed&&dnskey_rdata_ed25519(g_zsk_ed,dkrd,sizeof(dkrd))>0){found=1;
            off=emit_rr(resp,off,resp_len,qname,DNS_TYPE_CDNSKEY,3600,dkrd,36,dnssec_ok,&answers);}
    }
    ...
    for(int i=0;i<static_zone_sz;i++){
        ...
        if(r->type!=qtype&&qtype!=DNS_TYPE_ANY)continue;
        uint8_t rd[256];uint16_t rdlen=0;
        switch(r->type){
        case DNS_TYPE_A:memcpy(rd,r->rdata_a,4);rdlen=4;break;
        case DNS_TYPE_AAAA:{struct in6_addr a6;if(inet_pton(AF_INET6,r->rdata_str,&a6)!=1)continue;
            memcpy(rd,&a6,16);rdlen=16;break;}
    ...
    if(qtype==DNS_TYPE_A||qtype==DNS_TYPE_ANY){
        ...
        if(vk_get(k,val,sizeof(val))){found=1;struct in_addr a4;if(inet_pton(AF_INET,val,&a4)==1){
            uint8_t rd[4];memcpy(rd,&a4,4);long tl=vk_ttl(k);if(tl<1)tl=1;
            off=emit_rr(resp,off,resp_len,qname,DNS_TYPE_A,(uint32_t)tl,rd,4,dnssec_ok,&answers);}}
    ...
    if(qtype==DNS_TYPE_AAAA||qtype==DNS_TYPE_ANY){
        ...
        if(vk_get(k,val,sizeof(val))){found=1;struct in6_addr a6;if(inet_pton(AF_INET6,val,&a6)==1){
            uint8_t rd[16];memcpy(rd,&a6,16);long tl=vk_ttl(k);if(tl<1)tl=1;
            off=emit_rr(resp,off,resp_len,qname,DNS_TYPE_AAAA,(uint32_t)tl,rd,16,dnssec_ok,&answers);}}
    ...
    {uint16_t pts[]={DNS_TYPE_CNAME,DNS_TYPE_MX,DNS_TYPE_TXT,DNS_TYPE_NS,
                     ...
                     DNS_TYPE_DNAME,DNS_TYPE_LOC,DNS_TYPE_URI,DNS_TYPE_NAPTR,0};
    for(int pi=0;pts[pi];pi++){uint16_t pt=pts[pi];
        if(qtype!=pt&&qtype!=DNS_TYPE_ANY)continue;
        char k[512];snprintf(k,sizeof(k),"zone:%s:%s",type2str(pt),qname);
        char val[512];if(!vk_get(k,val,sizeof(val)))continue;found=1;
        uint32_t ttl=DEFAULT_TTL;char *pipe=strchr(val,'|');
        if(pipe){ttl=(uint32_t)atoi(val);pipe++;}else pipe=val;
        uint8_t rd[512];uint16_t rdlen=0;
        switch(pt){
        case DNS_TYPE_CNAME:case DNS_TYPE_NS:case DNS_TYPE_DNAME:{
            int n=name_to_wire(pipe,rd,sizeof(rd));if(n<0)continue;rdlen=(uint16_t)n;break;}
        case DNS_TYPE_MX:{char *sp=strchr(pipe,'|');uint16_t pref=10;
        ...
        case DNS_TYPE_URI:{/* ttl|priority|weight|target */
            ...
            rd[0]=prio>>8;rd[1]=prio&0xFF;rd[2]=weight>>8;rd[3]=weight&0xFF;
            memcpy(rd+4,tok,tlen);rdlen=(uint16_t)(4+tlen);break;}
        case DNS_TYPE_NAPTR:{/* ttl|order|pref|flags|service|regexp|replacement */
            ...
            int fl=(int)strlen(flags2);rd[rp2++]=(uint8_t)fl;memcpy(rd+rp2,flags2,fl);rp2+=fl;
            int sl2=(int)strlen(svc2);rd[rp2++]=(uint8_t)sl2;memcpy(rd+rp2,svc2,sl2);rp2+=sl2;
            int rl=(int)strlen(re2);rd[rp2++]=(uint8_t)rl;memcpy(rd+rp2,re2,rl);rp2+=rl;
            int nn2=name_to_wire(repl2[0]?repl2:".",rd+rp2,sizeof(rd)-rp2);
            if(nn2>0)rp2+=nn2;rdlen=(uint16_t)rp2;break;}
        default:continue;}
    ...
    if(!found&&answers==0&&qtype!=DNS_TYPE_CNAME&&qtype!=DNS_TYPE_ANY){
        char cname_key[512];snprintf(cname_key,sizeof(cname_key),"zone:CNAME:%s",qname);
        char cname_val[512];
        if(vk_get(cname_key,cname_val,sizeof(cname_val))){
            ...
            if(cpipe)cname_ttl=(uint32_t)atoi(cname_val);
            uint8_t crd[256];int crlen=name_to_wire(cname_target,crd,sizeof(crd));
            if(crlen>0){
                ...
                for(int hop=0;hop<8&&cur[0];hop++){
                    ...
                    if(qtype==DNS_TYPE_A||qtype==DNS_TYPE_ANY){
                        char ck[512];snprintf(ck,sizeof(ck),"zone:A:%s",cur);
                        char cv[256];
    ...
    if(!found){
        ...
        if(dot){
            ...
            for(int i=0;i<static_zone_sz&&!found;i++){
                ...
                if(!streq_ci(r->name,wname))continue;
                found=1; /* wildcard owner exists */
                if(r->type!=qtype&&qtype!=DNS_TYPE_ANY)continue;
                uint8_t rd[256];uint16_t rdlen=0;
                switch(r->type){
                case DNS_TYPE_A:memcpy(rd,r->rdata_a,4);rdlen=4;break;
                case DNS_TYPE_AAAA:{struct in6_addr a6w;
                    if(inet_pton(AF_INET6,r->rdata_str,&a6w)!=1)continue;
                    memcpy(rd,&a6w,16);rdlen=16;break;}
                case DNS_TYPE_CNAME:case DNS_TYPE_NS:case DNS_TYPE_DNAME:{
                    int nw=name_to_wire(r->rdata_str,rd,sizeof(rd));
                    if(nw<0)continue;rdlen=(uint16_t)nw;break;}
                case DNS_TYPE_MX:{rd[0]=r->rdata_pref>>8;rd[1]=r->rdata_pref&0xFF;
            ...
            if(!found){
                ...
                if(qtype==DNS_TYPE_A||qtype==DNS_TYPE_ANY){
                    ...
                    char wv[128];
                    if(vk_get(wk,wv,sizeof(wv))){found=1;
                        uint32_t wttl=DEFAULT_TTL;char *wp=strchr(wv,'|');
                        if(wp){wttl=(uint32_t)atoi(wv);wp++;}else wp=wv;
                        struct in_addr wa4;if(inet_pton(AF_INET,wp,&wa4)==1){
                            uint8_t rd[4];memcpy(rd,&wa4,4);
                            off=emit_rr(resp,off,resp_len,qname,DNS_TYPE_A,wttl,rd,4,dnssec_ok,&answers);}}}
                if(qtype==DNS_TYPE_AAAA||qtype==DNS_TYPE_ANY){
                    ...
                    if(vk_get(wk,wv,sizeof(wv))){found=1;
                        uint32_t wttl=DEFAULT_TTL;char *wp=strchr(wv,'|');
                        if(wp){wttl=(uint32_t)atoi(wv);wp++;}else wp=wv;
                        struct in6_addr wa6;if(inet_pton(AF_INET6,wp,&wa6)==1){
                            uint8_t rd[16];memcpy(rd,&wa6,16);
                            off=emit_rr(resp,off,resp_len,qname,DNS_TYPE_AAAA,wttl,rd,16,dnssec_ok,&answers);}}}
            }
        }
    }
...
finish_answer:
    ...
    int ede_code=-1;const char *ede_text=NULL;
    if(answers==0){
        /* RFC 2308: add SOA in authority for NXDOMAIN / NODATA */
        if(!found){
            rh->flags=htons(DNS_QR|DNS_AA|DNS_RCODE_NXDOMAIN);
            ede_code=EDE_NXDOMAIN;ede_text="Name does not exist in zone";
    ...
    if(any_minimal&&answers>0){
        ...
        append_rr(resp,off,resp_len,qname,13/*HINFO*/,DNS_CLASS_IN,3600,hinfo_rd,sizeof(hinfo_rd));
...
static int handle_update(const uint8_t *pkt,int plen,uint8_t *resp){
    ...
    const dns_hdr_t *h=(const dns_hdr_t*)pkt;
    dns_hdr_t *rh=(dns_hdr_t*)resp;
    rh->id=h->id;rh->flags=htons(DNS_QR|DNS_OPCODE_UPDATE|DNS_RCODE_NOERROR);
    rh->qdcount=rh->ancount=rh->nscount=rh->arcount=0;
    /* TSIG verification */
    if(g_tsig_secret_len>0&&!tsig_verify(pkt,plen)){
        rh->flags=htons(DNS_QR|DNS_OPCODE_UPDATE|DNS_RCODE_BADSIG);return 12;}
    int off=12;
    for(int i=0;i<ntohs(h->qdcount);i++){
        char zn[256];int za=name_from_wire(pkt,plen,off,zn,sizeof(zn));
        if(za<0)goto formerr;
    ...
    for(int i=0;i<ntohs(h->nscount);i++){
        ...
        if(a<0||a+9>plen)goto formerr;
        uint16_t ut=get16(pkt,a),uc=get16(pkt,a+2);uint32_t uttl=get32(pkt,a+4);
        uint16_t rdlen=get16(pkt,a+8);off=a+10;if(off+rdlen>plen)goto formerr;
        const uint8_t *rd=pkt+off;off+=rdlen;char k[512];
        if(uc==DNS_CLASS_IN&&rdlen>0){
            if(ut==DNS_TYPE_A&&rdlen==4){char ip[32];snprintf(ip,sizeof(ip),"%d.%d.%d.%d",rd[0],rd[1],rd[2],rd[3]);
                snprintf(k,sizeof(k),"ddns:A:%s",un);vk_set(k,ip,uttl?uttl:DEFAULT_TTL);
            ...
            else if(ut==DNS_TYPE_AAAA&&rdlen==16){char ip6[INET6_ADDRSTRLEN];
                inet_ntop(AF_INET6,rd,ip6,sizeof(ip6));
                snprintf(k,sizeof(k),"ddns:AAAA:%s",un);vk_set(k,ip6,uttl?uttl:DEFAULT_TTL);
                {uint32_t prev=g_soa_serial;uint32_t next=serial_bump();
...
formerr:rh->flags=htons(DNS_QR|DNS_OPCODE_UPDATE|DNS_RCODE_FORMERR);return 12;}
...
static void ixfr_journal_append(uint32_t from_serial,uint32_t to_serial,
                                char op,const char *name,const char *value){
    char entry[1024];
    ...
    if(valkey_ensure(&vk)>=0){
        ...
        if(r.type==3&&r.integer>IXFR_JOURNAL_MAX){
            char trim[32];snprintf(trim,sizeof(trim),"%d",IXFR_JOURNAL_MAX);
            resp_reply_t r2;
            char ltrim_idx[32];snprintf(ltrim_idx,sizeof(ltrim_idx),"%d",-IXFR_JOURNAL_MAX);
...
static int ixfr_journal_fetch(uint32_t from_serial,
                              char **out_entries,int max_entries){
    pthread_mutex_lock(&g_vk_mutex);
    if(valkey_ensure(&vk)<0){pthread_mutex_unlock(&g_vk_mutex);return -1;}
    resp_reply_t r;
    resp_cmd(&vk,&r,4,"LRANGE",IXFR_JOURNAL_KEY,"0","-1");
    if(r.type!=5){pthread_mutex_unlock(&g_vk_mutex);return -1;}
    int count=r.count,found=-1,n=0;
    for(int i=0;i<count;i++){
        resp_reply_t er;resp_parse(&vk,&er);
...
static int axfr_ip_allowed(const struct in_addr *cip){
    ...
    while(tok){while(*tok==' ')tok++;
        ...
        if(slash){
            int bits=atoi(slash+1);*slash=0;
            if(bits<0)bits=0;if(bits>32)bits=32;
            struct in_addr net;inet_pton(AF_INET,tok,&net);
...
static int tcp_send_msg(int fd,SSL *ssl,const uint8_t *msg,int len){
    uint8_t lb[2];lb[0]=len>>8;lb[1]=len&0xFF;
    if(ssl){if(SSL_write(ssl,lb,2)<=0||SSL_write(ssl,msg,len)<=0)return -1;}
    else{if(send(fd,lb,2,0)!=2||send(fd,msg,len,0)!=len)return -1;}
    return 0;}

static void *axfr_thread(void *arg){
    axfr_conn_t c;memcpy(&c,arg,sizeof(c));free(arg);
    char cip[INET_ADDRSTRLEN];inet_ntop(AF_INET,&c.addr.sin_addr,cip,sizeof(cip));
    ...
    if(!axfr_ip_allowed(&c.addr.sin_addr)){
        ...
        uint8_t ref[12]={0};dns_hdr_t *rh=(dns_hdr_t*)ref;
        rh->id=0;rh->flags=htons(DNS_QR|DNS_AA|DNS_RCODE_REFUSED);
        tcp_send_msg(c.fd,c.ssl,ref,12);goto done;}

    uint8_t soa_rd[512];int soa_len=build_soa_rdata(soa_rd,sizeof(soa_rd));

    /* ── IXFR path (RFC 1995) ── */
    if(c.ixfr&&c.ixfr_serial>0&&c.ixfr_serial<g_soa_serial){
        char *entries[IXFR_JOURNAL_MAX];int ne=0;
        ...
        if(ne>0){
            ...
            for(int ei=0;ei<ne;ei++){
                char *e=entries[ei];if(!e)continue;
                uint32_t fs=0,ts=0;char op='A';char ename[256]="";char eval[256]="";
    ...
    if(c.ssl){SSL_shutdown(c.ssl);SSL_free(c.ssl);}close(c.fd);
    return NULL;}
...
static void notify_send(void){
    ...
    while(tok){while(*tok==' ')tok++;
        ...
        h->qdcount=htons(1);
        int off=12;int n=name_to_wire(g_zone_name,pkt+off,sizeof(pkt)-off);
 ...
 * ======================================================================= */
static int dns_process(const uint8_t *pkt,int plen,uint8_t *resp,int resp_len,
                       int is_tcp, const struct in_addr *cip){
    if(plen<12)return -1;
    const dns_hdr_t *h=(const dns_hdr_t*)pkt;
    uint16_t op=ntohs(h->flags)&DNS_OPCODE_MASK;
    if(op==DNS_OPCODE_QUERY)return build_query_resp(pkt,plen,resp,resp_len,is_tcp,cip);
    if(op==DNS_OPCODE_UPDATE)return handle_update(pkt,plen,resp);
    if(op==DNS_OPCODE_NOTIFY){
        /* Accept and acknowledge NOTIFY */
        dns_hdr_t *rh=(dns_hdr_t*)resp;rh->id=h->id;
        rh->flags=htons(DNS_QR|DNS_OPCODE_NOTIFY|DNS_RCODE_NOERROR);
        rh->qdcount=rh->ancount=rh->nscount=rh->arcount=0;
        dns_log(LOG_NOTICE,"[NOTIFY] Received NOTIFY\n");return 12;}
    dns_hdr_t *rh=(dns_hdr_t*)resp;rh->id=h->id;rh->flags=htons(DNS_QR|DNS_RCODE_NOTIMP);
    rh->qdcount=rh->ancount=rh->nscount=rh->arcount=0;return 12;}

/* ==========================================================================
 * DNS-over-TLS thread (RFC 7858) — handles AXFR too
 * ======================================================================= */
typedef struct{int fd;struct sockaddr_in addr;}dot_conn_t;
static void *dot_thread(void *arg){
    dot_conn_t c;memcpy(&c,arg,sizeof(c));free(arg);
    SSL *ssl=NULL;
    pthread_mutex_lock(&g_tls_mutex);SSL_CTX *ctx=g_dot_ctx;
    if(ctx){ssl=SSL_new(ctx);SSL_set_fd(ssl,c.fd);}pthread_mutex_unlock(&g_tls_mutex);
    if(ssl&&SSL_accept(ssl)<=0){ERR_print_errors_fp(stderr);SSL_free(ssl);close(c.fd);return NULL;}
    char cip[INET_ADDRSTRLEN];inet_ntop(AF_INET,&c.addr.sin_addr,cip,sizeof(cip));
    struct timeval tv={.tv_sec=30};setsockopt(c.fd,SOL_SOCKET,SO_RCVTIMEO,&tv,sizeof(tv));
    for(;;){
        uint8_t lb[2];int got=0;
        while(got<2){int n=ssl?SSL_read(ssl,lb+got,2-got):(int)recv(c.fd,lb+got,2-got,0);if(n<=0)goto dot_done;got+=n;}
        uint16_t ml=(uint16_t)((lb[0]<<8)|lb[1]);if(ml<12||ml>BUF_SIZE)break;
        uint8_t pkt[BUF_SIZE];got=0;
        while(got<ml){int n=ssl?SSL_read(ssl,pkt+got,ml-got):(int)recv(c.fd,pkt+got,ml-got,0);if(n<=0)goto dot_done;got+=n;}
        /* Check for AXFR */
        uint16_t qtype=0;
        {char qn[256]="";int a=name_from_wire(pkt,ml,12,qn,sizeof(qn));
         if(a>=0&&a+1<ml)qtype=get16(pkt,a);}
        if(qtype==252/*AXFR*/||qtype==251/*IXFR*/){
            axfr_conn_t *ac=malloc(sizeof(axfr_conn_t));
            if(!ac){if(ssl){SSL_shutdown(ssl);SSL_free(ssl);}close(c.fd);return NULL;}
            ac->fd=c.fd;ac->ssl=ssl;ac->addr=c.addr;
            ...
            if(qtype==251&&ml>12){
                ...
                int sc=12;char sn2[256];
                sc=name_from_wire(pkt,ml,sc,sn2,sizeof(sn2));
                if(sc>0)sc+=4;/* skip qtype+qclass */
                ...
                for(int i=0;i<ns_count&&sc+10<ml;i++){
                    char rn2[256];int ra=name_from_wire(pkt,ml,sc,rn2,sizeof(rn2));
                    if(ra<0)break;
                    ...
                    if(rt2==DNS_TYPE_SOA&&rdl2>=20){
                        ...
                        int sp=ra+10;
                        char mn2[256];sp=name_from_wire(pkt,ml,sp,mn2,sizeof(mn2));
                        if(sp>0){char rn3[256];sp=name_from_wire(pkt,ml,sp,rn3,sizeof(rn3));}
                        if(sp>0&&sp+4<=ml)ac->ixfr_serial=get32(pkt,sp);
                        break;
                    }
                    sc=ra+10+rdl2;
                }
        ...
        struct timespec _dt0,_dt1;clock_gettime(CLOCK_MONOTONIC,&_dt0);
        uint8_t resp[BUF_SIZE];int rlen=dns_process(pkt,ml,resp,sizeof(resp),1,&c.addr.sin_addr);
        clock_gettime(CLOCK_MONOTONIC,&_dt1);
        long _drtt=(long)((_dt1.tv_sec-_dt0.tv_sec)*1000000L+(_dt1.tv_nsec-_dt0.tv_nsec)/1000L);
        if(rlen<0)break;
        if(rlen>=4){uint8_t _rc=ntohs(get16(resp,2))&0xF;int _an=rlen>=8?ntohs(get16(resp,6)):0;
            qlog_write(cip,qn,qt,_rc,_an,_drtt,"dot");}
        uint8_t rlb[2];rlb[0]=rlen>>8;rlb[1]=rlen&0xFF;
        if(ssl){SSL_write(ssl,rlb,2);SSL_write(ssl,resp,rlen);}
        else{send(c.fd,rlb,2,0);send(c.fd,resp,rlen,0);}}
dot_done:
...
int main(int argc,char **argv){
    ...
    /* Phase 2b: Populate multi-zone table */
    {
        ...
        zone_upsert(g_zone_name,
            g_soa_mname[0]?g_soa_mname:mname,
            g_soa_rname[0]?g_soa_rname:rname,
            g_soa_serial,g_soa_refresh,g_soa_retry,
            g_soa_expire,g_soa_minimum,
            g_axfr_allow,g_notify_targets);
    }
    ...
    /* Plain TCP DNS on dns_port (RFC 1035 §4.2.2 — TC retry) */
    {
        ...
        if(ts>=0){
            ...
            struct sockaddr_in sa4t={.sin_family=AF_INET,
                .sin_port=htons(g_dns_port),.sin_addr.s_addr=INADDR_ANY};
            if(bind(ts,(struct sockaddr*)&sa4t,sizeof(sa4t))<0||listen(ts,32)<0)
                {perror("[TCP-DNS] bind (non-fatal)");close(ts);}
            else{tcp_dns_sock=ts;
    ...
    if(dns_sock<0){perror("dns socket");return 1;}
    setsockopt(dns_sock,SOL_SOCKET,SO_REUSEADDR,&opt,sizeof(opt));
    {struct sockaddr_in sa={.sin_family=AF_INET,.sin_port=htons(g_dns_port),.sin_addr.s_addr=INADDR_ANY};
     if(bind(dns_sock,(struct sockaddr*)&sa,sizeof(sa))<0){perror("dns bind");return 1;}}
    int dot_sock=socket(AF_INET,SOCK_STREAM,0);
    if(dot_sock<0){perror("dot socket");return 1;}
    ...
    for(;;){
        ...
        if(ready<0){if(errno==EINTR){
            /* EINTR from SIGHUP — check reload flag */
            if(g_reload_flag){g_reload_flag=0;
        ...
        if(FD_ISSET(dns_sock,&fds)){
            ...
            if(nn>0){char cip2[INET_ADDRSTRLEN];inet_ntop(AF_INET,&cli.sin_addr,cip2,sizeof(cip2));
                char qn[256]="?";uint16_t qt=0;
                int a=name_from_wire(pkt,nn,12,qn,sizeof(qn));if(a>=0&&a+1<nn)qt=get16(pkt,a);
                dns_log(LOG_DEBUG,"[DNS ] %s  %s %s\n",cip2,type2str(qt),qn);
                struct timespec _qt0,_qt1;clock_gettime(CLOCK_MONOTONIC,&_qt0);
                int rlen=dns_process(pkt,(int)nn,resp,sizeof(resp),0,&cli.sin_addr);
                clock_gettime(CLOCK_MONOTONIC,&_qt1);
                long _rtt=(long)((_qt1.tv_sec-_qt0.tv_sec)*1000000L+(_qt1.tv_nsec-_qt0.tv_nsec)/1000L);
                if(rlen>0){
        ...
        if(dns6_sock>=0&&FD_ISSET(dns6_sock,&fds)){
            ...
            ssize_t nn6=recvfrom(dns6_sock,pkt6,sizeof(pkt6),0,(struct sockaddr*)&cli6,&cl6);
            if(nn6>0){
                char cip6[INET6_ADDRSTRLEN];inet_ntop(AF_INET6,&cli6.sin6_addr,cip6,sizeof(cip6));
                char qn6[256]="?";uint16_t qt6=0;
                int a6=name_from_wire(pkt6,nn6,12,qn6,sizeof(qn6));if(a6>=0&&a6+1<(int)nn6)qt6=get16(pkt6,a6);
                ...
                long _rtt6=(long)((_t1.tv_sec-_t0.tv_sec)*1000000L+(_t1.tv_nsec-_t0.tv_nsec)/1000L);
                if(rlen6>0){
