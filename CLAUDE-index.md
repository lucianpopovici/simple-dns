...
| `CLAUDE-ENUM.md` | dnsd + apid | ADR-001 (ratified), Add 6 (ENUM, done) + provisioning helper (done 2026-07-06, `POST /enum/provision` in apid, `make check-enum-provision`) + bind config + ENUM roadmap — two-instance topology, ops (systemd/dashboard/parent DS), two-instance CI, and the NP/EPP-federation roadmap items remain open |
...
| `specs/CLAUDE-certd.md` | certd | PKI: ACME/EST extraction (done) + ARI (Add 1, done) + tls-alpn-01/RFC 8738 IP identifiers (Add 2/3, done 2026-07-06, `make check-acme-challenges`) + RFC 8659 CAA pre-flight (done 2026-07-06, `make check-caa`; TLSA integration was already done elsewhere — dnsd, mistracked here) + RFC 8739 STAR recurrent orders (done 2026-07-06, `make check-star`) — **entire checklist closed**, archived untracked |
