...
static void cert_current_load(void){
    ...
    blob[0]=0;
    if(!vk_get("cert:current",blob,sizeof(blob))||!blob[0])return;
    static char cert[MAX_PEM],key[MAX_PEM];
    if(cert_current_split(blob,cert,sizeof(cert),key,sizeof(key))<0)return;
...
static int acme_nonce_fetch(const char *host,int port){
    char path[512];int p2;char h2[256];
    parse_url(g_acme_dir_newnonce,h2,&p2,path,sizeof(path));
    char hdrs[4096]={0};int code=0;
    char *b=https_req_mtls(h2,p2,"HEAD",path,NULL,NULL,NULL,NULL,NULL,&code,hdrs,sizeof(hdrs));free(b);
...
static char *acme_post(const char *url,const char *payload,const char *host,int port,
                       int *code,char *rh,int rhl){
    acme_nonce_fetch(host,port);char *jws=acme_jws(g_acme_key,url,g_acme_nonce,payload);
    if(!jws)return NULL;
    char path[512];int p2;char h2[256];parse_url(url,h2,&p2,path,sizeof(path));
...
static char *acme_gen_csr(const char *domain,EVP_PKEY **dkout){
    ...
    if(!cb64){free(der);EVP_PKEY_free(*dkout);*dkout=NULL;return NULL;}
    b64url_enc(der,derlen,cb64,(int)((size_t)derlen*2+4));free(der);return cb64;}
...
static void load_config(void){
    ...
    char val[512];
    if(vk_get("config:acme_domain",val,sizeof(val))&&val[0])safe_strcpy(g_acme_domain,val,sizeof(g_acme_domain));
    if(vk_get("config:acme_email",val,sizeof(val))&&val[0])safe_strcpy(g_acme_email,val,sizeof(g_acme_email));
    if(vk_get("config:acme_ca",val,sizeof(val))&&val[0])safe_strcpy(g_acme_ca,val,sizeof(g_acme_ca));
    ...
    if(vk_get("config:est_server",val,sizeof(val))&&val[0])safe_strcpy(g_est_server,val,sizeof(g_est_server));
    if(vk_get("config:est_domain",val,sizeof(val))&&val[0])safe_strcpy(g_est_domain,val,sizeof(g_est_domain));
...
static int renewal_check(void){
    ...
    if(!g_acme_domain[0]&&!g_est_server[0]){
        dns_log(LOG_INFO,"[PKI] Nothing configured (config:acme_domain / config:est_server empty)\n");
        return 0;}
    if(!est_needs_renewal()&&!acme_needs_renewal())return 0;
...
int main(int argc,char **argv){
    ...
    for(int i=1;i<argc;i++){
        ...
        else{fprintf(stderr,"usage: %s [--once]\n",argv[0]);return 2;}}
