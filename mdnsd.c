...
typedef struct{int type;long integer;char str[4096];int count;}resp_reply_t;
...
static const char *type2str(uint16_t t){
    switch(t){case DNS_TYPE_A:return"A";case DNS_TYPE_AAAA:return"AAAA";
    case DNS_TYPE_PTR:return"PTR";case DNS_TYPE_SRV:return"SRV";
    case DNS_TYPE_TXT:return"TXT";case DNS_TYPE_CNAME:return"CNAME";
    default:return"?";}}
...
static int mdns_put_name(uint8_t *buf, int off, int blen, const char *name){
    ...
    char *sp=NULL;char *lbl=strtok_r(tmp,".",&sp);
    while(lbl){
...
static int mdns_lookup_records(uint8_t *buf, int off, int blen,
    ...
    for(int ti=0;ti<ntypes;ti++){
        ...
        for(int pi=0;pi<2;pi++){
            ...
            if(legacy&&ttl>10)ttl=10;
            uint8_t rd[512]; uint16_t rdlen=0;
            switch(qt){
            case DNS_TYPE_A:{
                struct in_addr a4;
                char *sp=NULL;char *ip=strtok_r(vptr,"|",&sp);
                while(ip){
            ...
            case DNS_TYPE_SRV:{
                ...
                char *sp=NULL;char *tok=strtok_r(vptr,"|",&sp); if(tok)prio=(uint16_t)atoi(tok);
                tok=strtok_r(NULL,"|",&sp); if(tok)weight=(uint16_t)atoi(tok);
                tok=strtok_r(NULL,"|",&sp); if(tok)port=(uint16_t)atoi(tok);
                tok=strtok_r(NULL,"|",&sp); if(tok)safe_strcpy(target,tok,sizeof(target));
...
static int mdns_build_response(const uint8_t *query, int qlen,
    ...
    for(int qi=0;qi<nq;qi++){
        ...
        if(qclasses[qi]!=DNS_CLASS_IN&&qclasses[qi]!=DNS_CLASS_ANY) continue;
        ...
        int new_off=mdns_lookup_records(resp,off,resp_len,qname,qtypes[qi],
                                        &ancount,is_legacy_unicast);
        if(new_off>0) off=new_off;
    }
        ...
        if(r.type==5){
            for(int i=0;i<r.count;i++){
                ...
                case DNS_TYPE_SRV:{uint16_t pr=0,wt=0,po=0;char tg[256]="";
                    char *sp=NULL;char *tok=strtok_r(vptr,"|",&sp);if(tok)pr=(uint16_t)atoi(tok);
                    tok=strtok_r(NULL,"|",&sp);if(tok)wt=(uint16_t)atoi(tok);
                    tok=strtok_r(NULL,"|",&sp);if(tok)po=(uint16_t)atoi(tok);
                    tok=strtok_r(NULL,"|",&sp);if(tok)safe_strcpy(tg,tok,sizeof(tg));
...
static int iface_allowed(const char *name){
    if(g_iface_count<0)return 1;           /* "all" explicitly configured */
    for(int i=0;i<g_iface_count;i++)
        if(strcmp(g_ifaces[i],name)==0)return 1;
    return 0;}
...
static int mdns_open_socket(int af){
    ...
    if(af==AF_INET){
        ...
        for(struct ifaddrs *i=ifa;i;i=i->ifa_next){
            ...
            if(!iface_allowed(i->ifa_name)) continue;
            struct ip_mreq mr={0};
