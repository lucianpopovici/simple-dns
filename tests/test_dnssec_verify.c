...
static const uint8_t WIRE_QNAME[] = {
    3,'w','w','w', 7,'e','x','a','m','p','l','e', 3,'c','o','m', 0
};
/* example.com in wire format (RRSIG signer name) */
static const uint8_t WIRE_SIGNER[] = {
...
static int build_signing_input(const uint8_t *rrsig_hdr, int hdr_len,
                               uint8_t *out) {
    int o = 0;
    memcpy(out + o, rrsig_hdr, (size_t)hdr_len); o += hdr_len;
    const uint8_t rdatas[2][4] = { {192,0,2,1}, {192,0,2,99} };
    for (int i = 0; i < 2; i++) {
        memcpy(out + o, WIRE_QNAME, sizeof(WIRE_QNAME)); o += (int)sizeof(WIRE_QNAME);
        put16(out, o, 1); o += 2;                 /* type A */
        put16(out, o, 1); o += 2;                 /* class IN */
        put32(out, o, ORIG_TTL); o += 4;          /* ORIGINAL ttl, not PKT_TTL */
        put16(out, o, 4); o += 2;                 /* rdlength */
        memcpy(out + o, rdatas[i], 4); o += 4;
    }
    return o;
}
...
static void build_packet(testpkt_t *t, uint8_t alg,
                         const uint8_t *sig,
    ...
    put16(p, 4, 1); put16(p, 6, 4); put16(p, 8, 0); put16(p, 10, 0);
    o = 12;
    /* question: www.example.com A IN  (qname at offset 12) */
    memcpy(p + o, WIRE_QNAME, sizeof(WIRE_QNAME)); o += (int)sizeof(WIRE_QNAME);
    put16(p, o, 1); o += 2; put16(p, o, 1); o += 2;
    /* answer 1: owner = compression pointer to qname, A 192.0.2.99
     * (out of canonical order on purpose; .99 sorts after .1) */
    t->rr_offs[0] = o;
    p[o++] = 0xC0; p[o++] = 0x0C;
    put16(p, o, 1); o += 2; put16(p, o, 1); o += 2;
    put32(p, o, PKT_TTL); o += 4;
    put16(p, o, 4); o += 2;
    t->a1_rdata_off = o;
    p[o++] = 192; p[o++] = 0; p[o++] = 2; p[o++] = 99;
    ...
    put32(p, o, PKT_TTL); o += 4;
    put16(p, o, 4); o += 2;
    p[o++] = 192; p[o++] = 0; p[o++] = 2; p[o++] = 1;
    /* answer 3: RRSIG covering the A RRset */
    int rdlen = 18 + (int)sizeof(WIRE_SIGNER) + 64;
    p[o++] = 0xC0; p[o++] = 0x0C;
    put16(p, o, DNS_TYPE_RRSIG); o += 2; put16(p, o, 1); o += 2;
    put32(p, o, PKT_TTL); o += 4;
    put16(p, o, (uint16_t)rdlen); o += 2;
    int rdstart = o;
    put16(p, o, 1); o += 2;            /* type covered: A */
    p[o++] = alg;                      /* algorithm */
    p[o++] = 3;                        /* labels: www.example.com = 3 */
    put32(p, o, ORIG_TTL); o += 4;     /* original TTL */
    put32(p, o, 0xFFFFFFFFu); o += 4;  /* expiration (not checked) */
    put32(p, o, 0); o += 4;            /* inception  (not checked) */
    put16(p, o, 0); o += 2;            /* key tag    (not checked) */
    memcpy(p + o, WIRE_SIGNER, sizeof(WIRE_SIGNER)); o += (int)sizeof(WIRE_SIGNER);
    t->sig_off = o;
    memcpy(p + o, sig, 64); o += 64;
    memcpy(t->rrsig_rd, p + rdstart, (size_t)rdlen);
    t->rrsig_rdlen = rdlen;
    /* answer 4: DNSKEY (owner example.com via pointer into the qname) */
    p[o++] = 0xC0; p[o++] = 0x10;
    put16(p, o, DNS_TYPE_DNSKEY); o += 2; put16(p, o, 1); o += 2;
    put32(p, o, PKT_TTL); o += 4;
    put16(p, o, (uint16_t)(4 + publen)); o += 2;
    int dkstart = o;
    put16(p, o, 0x0101); o += 2;       /* flags: ZK + SEP */
    p[o++] = 3;                        /* protocol */
    p[o++] = alg;
    memcpy(p + o, pub, (size_t)publen); o += publen;
...
static int sign_data(EVP_PKEY *pkey, int is_ed,
                     const uint8_t *data, size_t dlen, uint8_t sig64[64]) {
    ...
    if (EVP_DigestSignInit(ctx, NULL, is_ed ? NULL : EVP_sha256(), NULL, pkey) > 0 &&
        ...
        if (is_ed) {
            if (slen == 64) { memcpy(sig64, der, 64); ok = 1; }
        } else {
            ...
            if (es) {
                ...
                if (BN_bn2binpad(r, sig64, 32) == 32 &&
                    BN_bn2binpad(s, sig64 + 32, 32) == 32) ok = 1;
                ECDSA_SIG_free(es);
            }
        }
    }
...
static void run_alg(uint8_t alg) {
    ...
    if (is_ed) {
        ...
        if (EVP_PKEY_get_octet_string_param(pkey,
                OSSL_PKEY_PARAM_ENCODED_PUBLIC_KEY, enc, sizeof(enc), &l) <= 0 ||
            l != 65 || enc[0] != 0x04) {
    ...
    int r = dnssec_verify_rrset(t.pkt, t.plen, t.rr_offs, 2,
                                t.rrsig_rd, t.rrsig_rdlen,
                                t.dnskey_rd, t.dnskey_rdlen);
    CHECK(r == 1, "intact RRset verifies (known-answer)");

    /* 2. negative: flip one bit of A rdata → must reject */
    t.pkt[t.a1_rdata_off + 3] ^= 0x01;
    r = dnssec_verify_rrset(t.pkt, t.plen, t.rr_offs, 2,
                            t.rrsig_rd, t.rrsig_rdlen,
                            t.dnskey_rd, t.dnskey_rdlen);
    ...
    /* 4. end-to-end through dnssec_validate_response with a trust anchor */
    {
        ...
        dnssec_status_t st = dnssec_validate_response(t.pkt, t.plen,
                                                      "www.example.com");
