...
static int valkey_connect(resp_conn_t *c){
    ...
    if(inet_pton(AF_INET,g_valkey_host,&sa.sin_addr)!=1){
        struct addrinfo hints={0},*res;char ps[8];snprintf(ps,sizeof(ps),"%d",g_valkey_port);
        hints.ai_family=AF_INET;hints.ai_socktype=SOCK_STREAM;
        if(getaddrinfo(g_valkey_host,ps,&hints,&res)!=0){close(c->fd);c->fd=-1;return -1;}
    ...
    resp_reply_t r;
    if(g_valkey_pass[0]){if(resp_cmd(c,&r,2,"AUTH",g_valkey_pass)<0||r.type==1){close(c->fd);c->fd=-1;return -1;}}
...
static int vk_get(const char *key,char *out,int olen){
    ...
    safe_strcpy(out,r.str,olen);return 1;}
...
static const char *type2str(uint16_t t){
    ...
    case 12:return"PTR";case 15:return"MX";case 16:return"TXT";
    case 28:return"AAAA";case 29:return"LOC";case 33:return"SRV";
    case 39:return"DNAME";case 44:return"SSHFP";case 52:return"TLSA";
    case 257:return"CAA";default:return"?";}}
static uint16_t str2type(const char *s){
    if(!strcasecmp(s,"A"))return 1;     if(!strcasecmp(s,"NS"))return 2;
    if(!strcasecmp(s,"CNAME"))return 5; if(!strcasecmp(s,"PTR"))return 12;
    if(!strcasecmp(s,"MX"))return 15;   if(!strcasecmp(s,"TXT"))return 16;
    if(!strcasecmp(s,"AAAA"))return 28; if(!strcasecmp(s,"LOC"))return 29;
    if(!strcasecmp(s,"SRV"))return 33;  if(!strcasecmp(s,"DNAME"))return 39;
    if(!strcasecmp(s,"SSHFP"))return 44;if(!strcasecmp(s,"TLSA"))return 52;
    if(!strcasecmp(s,"CAA"))return 257; return 0;}
...
static void tls_reload(void){
    ...
    if(g_https_ctx){SSL_CTX_free(g_https_ctx);g_https_ctx=NULL;}
    g_https_ctx=tls_ctx_from_pem(g_tls_cert_pem,g_tls_key_pem,g_mtls_ca_pem,
                                 g_mtls_ca_pem[0]?1:0);
    if(g_https_ctx){
    ...
    dns_log(LOG_INFO,"[TLS] HTTPS context %s (mTLS %s)\n",
            g_https_ctx?"loaded":"unavailable (no cert yet)",
            g_mtls_ca_pem[0]?"on":"off");}
...
static void *cert_watch_thread(void *arg){(void)arg;
    for(;;){
        ...
        if(strcmp(ca2,g_mtls_ca_pem)){safe_strcpy(g_mtls_ca_pem,ca2,sizeof(g_mtls_ca_pem));changed=1;}
        if(changed){
...
static void api_send(int fd,SSL *ssl,int code,const char *body){
    ...
    const char *st=code==200?"OK":code==201?"Created":code==400?"Bad Request":
                   ...
                   code==204?"No Content":code==502?"Bad Gateway":
...
static int dns_forward(const uint8_t *q,int qlen,uint8_t *resp,int rmax){
    ...
    close(fd);
    if(rlen<12)return -1;
    if(!(resp[2]&0x02))return rlen;        /* no TC — done */
    /* Truncated: retry over TCP */
    int tfd=socket(AF_INET,SOCK_STREAM,0);
    if(tfd<0)return rlen;
    setsockopt(tfd,SOL_SOCKET,SO_RCVTIMEO,&tv,sizeof(tv));
    setsockopt(tfd,SOL_SOCKET,SO_SNDTIMEO,&tv,sizeof(tv));
    if(connect(tfd,(struct sockaddr*)&sa,sizeof(sa))!=0){close(tfd);return rlen;}
    uint8_t lp[2]={(uint8_t)(qlen>>8),(uint8_t)(qlen&0xFF)};
    if(send(tfd,lp,2,MSG_NOSIGNAL)!=2||send(tfd,q,qlen,MSG_NOSIGNAL)!=qlen){
        close(tfd);return rlen;}
    uint8_t lr[2];int got=0;
    while(got<2){ssize_t n=recv(tfd,lr+got,2-got,0);if(n<=0){close(tfd);return rlen;}got+=(int)n;}
    int tlen=(lr[0]<<8)|lr[1];
    if(tlen<12||tlen>rmax){close(tfd);return rlen;}
    got=0;
    while(got<tlen){ssize_t n=recv(tfd,resp+got,tlen-got,0);if(n<=0){close(tfd);return rlen;}got+=(int)n;}
    close(tfd);
    return tlen;}
...
static void handle_metrics(int fd,SSL *ssl){
    char dnsd_http[16];snprintf(dnsd_http,sizeof(dnsd_http),"%d",g_http_port);
    char port_s[64];int dport=0;
    if(vk_get("config:metrics_port",port_s,sizeof(port_s))&&port_s[0])dport=atoi(port_s);
    if(dport<=0)dport=g_http_port+1;   /* dnsd default: http_port+1 */
    int tfd=socket(AF_INET,SOCK_STREAM,0);
    if(tfd<0){api_send(fd,ssl,502,"metrics unavailable\n");return;}
    struct timeval tv={.tv_sec=3};
    setsockopt(tfd,SOL_SOCKET,SO_RCVTIMEO,&tv,sizeof(tv));
    setsockopt(tfd,SOL_SOCKET,SO_SNDTIMEO,&tv,sizeof(tv));
    ...
    if(connect(tfd,(struct sockaddr*)&sa,sizeof(sa))!=0){
        close(tfd);api_send(fd,ssl,502,"metrics unavailable\n");return;}
    const char *req="GET /metrics HTTP/1.0\r\nHost: localhost\r\nConnection: close\r\n\r\n";
    if(send(tfd,req,strlen(req),MSG_NOSIGNAL)<0){close(tfd);api_send(fd,ssl,502,"metrics unavailable\n");return;}
    char buf[HTTP_BUF];int total=0;
    for(;;){ssize_t n=recv(tfd,buf+total,sizeof(buf)-1-total,0);
        if(n<=0)break;total+=(int)n;if(total>=(int)sizeof(buf)-1)break;}
    close(tfd);buf[total]=0;
    if(total<=0){api_send(fd,ssl,502,"metrics unavailable\n");return;}
    /* relay dnsd's complete HTTP response as-is */
    if(ssl)SSL_write(ssl,buf,total);else{ssize_t w=write(fd,buf,total);(void)w;}}
...
static void handle_api(int fd,SSL *ssl,int is_mgmt){
    ...
    if(!strcasecmp(path,"/dns-query")){
        if(!strcasecmp(method,"POST")){
            ...
            uint8_t pkt[DNS_BUF];int plen=b64url_dec(dns_b64,pkt,sizeof(pkt));
            if(plen<12){api_send(fd,ssl,400,"bad dns parameter\n");return;}
            handle_doh(fd,ssl,pkt,plen);}
        return;}

    if(!strcmp(path,"/health")){api_send(fd,ssl,200,"ok\n");return;}
    if(!strcmp(path,"/metrics")){handle_metrics(fd,ssl);return;}
    ...
    if(!is_mgmt){
        ...
        if(!secret[0]||strcmp(akey,secret)!=0){api_send(fd,ssl,403,"forbidden\n");return;}}
    ...
    if(!strcmp(path,"/zone")&&(!strcasecmp(method,"POST")||!strcasecmp(method,"PUT"))){
        ...
        if(rt==15)snprintf(vval,sizeof(vval),"%u|%s|%s",ttl,pref[0]?pref:"10",value);
        else snprintf(vval,sizeof(vval),"%u|%s",ttl,value);
        vk_set(vkey,vval,0);serial_bump();
        dns_log(LOG_NOTICE,"[ZONE] %s %s = %s\n",type2str(rt),name,vval);
        char rb[600];snprintf(rb,sizeof(rb),"ok: %s\n",vkey);api_send(fd,ssl,201,rb);return;}
    if(!strcmp(path,"/zone")&&!strcasecmp(method,"DELETE")){
        ...
        serial_bump();char rb[128];snprintf(rb,sizeof(rb),"ok: deleted %d record(s)\n",d);api_send(fd,ssl,200,rb);return;}
    ...
    if(is_mgmt&&!strcmp(path,"/config")&&!strcasecmp(method,"POST")){
        ...
        char fullkey[160];snprintf(fullkey,sizeof(fullkey),"config:%s",cfgkey);
        vk_set(fullkey,cfgval,0);
        char rb[320];snprintf(rb,sizeof(rb),
            "ok: config:%s updated (dnsd applies on SIGHUP until Step 6)\n",cfgkey);
        api_send(fd,ssl,200,rb);return;}

    /* Multi-zone management (zone_table:* — the format dnsd loads at boot) */
    if(is_mgmt&&!strcasecmp(method,"GET")&&!strcmp(path,"/zone/list")){
        char body[HTTP_BUF];int bp=0;
        pthread_mutex_lock(&g_vk_mutex);
        if(valkey_ensure(&vk)>=0){resp_reply_t r;resp_cmd(&vk,&r,2,"KEYS","zone_table:*");
            if(r.type==5){bp+=snprintf(body+bp,sizeof(body)-bp,"zones: %d\n",r.count);
                for(int i=0;i<r.count&&bp<(int)sizeof(body)-512;i++){
                    resp_reply_t kr;resp_parse(&vk,&kr);if(kr.type!=2)continue;
                    resp_reply_t vr;resp_cmd(&vk,&vr,2,"GET",kr.str);
                    bp+=snprintf(body+bp,sizeof(body)-bp,"  %-40s = %s\n",
                        kr.str+11,vr.type==2?vr.str:"?");}}
            else bp+=snprintf(body+bp,sizeof(body)-bp,"zones: 0\n");}
        else bp+=snprintf(body+bp,sizeof(body)-bp,"valkey unavailable\n");
        pthread_mutex_unlock(&g_vk_mutex);
    ...
    if(is_mgmt&&!strcasecmp(method,"POST")&&!strcmp(path,"/zone/add")){
        ...
        char rb[320];snprintf(rb,sizeof(rb),"ok: zone %s added\n",zn);
    ...
    if(is_mgmt&&!strcasecmp(method,"POST")&&!strcmp(path,"/zone/del")){
        ...
        if(!zn[0]){api_send(fd,ssl,400,"name required\n");return;}
        char zkey[300];snprintf(zkey,sizeof(zkey),"zone_table:%s",zn);
        if(!vk_del(zkey)){api_send(fd,ssl,404,"zone not found\n");return;}
...
static void *http_thread(void *arg){(void)arg;
...
static void *https_thread(void *arg){(void)arg;
    ...
    for(;;){
        ...
        int is_mgmt=0;
        if(peer){X509_free(peer);is_mgmt=(SSL_get_verify_result(ssl)==X509_V_OK);}
...
static void load_config(void){
    ...
    char val[64];
    if(vk_get("config:http_port",val,sizeof(val))&&val[0])g_http_port=atoi(val);
    if(vk_get("config:https_port",val,sizeof(val))&&val[0])g_https_port=atoi(val);
    if(vk_get("config:dns_port",val,sizeof(val))&&val[0])g_dns_port=atoi(val);
...
int main(int argc,char **argv){
    ...
    dns_log(LOG_NOTICE,"[apid] Starting: HTTP :%d, HTTPS :%d, dnsd at 127.0.0.1:%d\n",
            g_http_port,g_https_port,g_dns_port);
    pthread_t t1,t2,t3;
    if(pthread_create(&t3,NULL,cert_watch_thread,NULL)==0)pthread_detach(t3);
    if(pthread_create(&t2,NULL,https_thread,NULL)==0)pthread_detach(t2);
